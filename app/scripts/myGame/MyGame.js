...
MyGame = {
    create: function() {
        ...
        for (var i = 0; i < 250; i++) {
            ...
            if (chance == 0) {
                if (whichTree==0) {
                    this.trees.create(xIndex + 50 + randomx, yChange - 185-500, 'tree1');
                }
                else if (whichTree==1) {
                    this.trees.create(xIndex + 50 + randomx, yChange - 175-500, 'tree2');
                }
                else if (whichTree==2) {
                    this.trees.create(xIndex + 50 + randomx, yChange - 175-500, 'tree3');
                }
            }
            if (lanternChance==0) {
        ...
        this.score = 0.0;
        this.scoreText = this.add.text(750, 10, '0.0 P', {fontSize: '32px', fill: '#000'});
        speedup=800;
        timestar=20;
        ...
        /*mute = this.add.text(0, 550, 'mute', {fontSize: '34px', fill: "#000"});
         mute.inputEnable = true;
         mute.events.onInputUp.add(function ()
         {
         if (this.sound.mute = 1)
         {
         this.sound.volume = 0;
         } else {
         this.sound.volume = 1;
    ...
    update: function()
    {
        ...
        this.physics.arcade.overlap(this.land, this.leftedge, this.starKill, null, this);
        ...
        if (!dead) {
            ...
            this.scoreText.text = this.score.toFixed(1) + ' P';
        }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    {
        if (star.y>tree.y+475 && star.y<tree.y+675&& tree.alpha==1) {
            star.kill();
    ...
    {
        if ((tree.alpha==1 && player.y>tree.y+500)||(player.y>300||(this.player.x<300 && tree.alpha==1))) {
            this.player.kill();
